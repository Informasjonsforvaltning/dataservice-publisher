"""Mapper module for mapping a catalog to rdf."""
...
class Contact:
    def __init__(self, document):
        assert document["name"] is not None, "the contact must have a name"
...
def _add_dataservice_to_graph(g: Graph, dataservice: DataService) -> Graph:
    ...
    if hasattr(dataservice, "title"):
        g.add(
            (URIRef(dataservice_uri), dct.title, Literal(dataservice.title, lang="nb"))
        )
    ...
    if hasattr(dataservice, "endpointURL"):
        g.add(
            (URIRef(dataservice_uri), dcat.endpointURL, URIRef(dataservice.endpointURL))
        )
        ...
        if hasattr(dataservice.contactpoint, "name"):
            g.add(
                (
                    ...
                    Literal(dataservice.contactpoint.name, lang="nb"),
                )
            )
        if hasattr(dataservice.contactpoint, "email"):
            g.add(
                (
                    contactpoint,
                    vcard.hasEmail,
                    URIRef("mailto:" + dataservice.contactpoint.email),
                )
            )
...
def map_catalogs_to_rdf(catalogs: List[Catalog], format="turtle") -> str:
    """Maps the list of catalogs to rdf and returns a serialization
...
def map_dataservices_to_rdf(dataservices: List[DataService], format="turtle") -> str:
    """Maps the list of dataservices to rdf and returns a serialization
       as a string according to format"""
...
def map_dataservice_to_rdf(dataservice: DataService, format="turtle") -> str:
    """Maps the dataservice to rdf and returns a serialization
       as a string according to format"""
